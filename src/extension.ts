...
class PandocFormatProvider
  ...
  configure(ctx: ExtensionContext, register: boolean) {
    const config = workspace.getConfiguration("panfmt");
    ...
    if (result.error) {
      ...
      if (result.status) {
        window.showWarningMessage(
          "panfmt: warnings when formatting string: " +
  ...
  formatString(text: string, ioFamily: string | undefined): string | undefined {
    if (this.available && ioFamily) {
      ...
      window.showErrorMessage(
        "panfmt: could not find pandoc executable",
      );
    }
    return undefined;
  }
  ...
  formatEdit(
    ...
    if (text.length > 0) {
      ...
      if (formatted && formatted.length > 0) {
        console.log(`panfmt: formatted document successfully (${langId})!`);
  ...
  register(ctx: ExtensionContext) {
    if (this.enabled) {
      for (let langId of ["markdown", "typst"]) {
        ...
        console.log(`panfmt: registered as ${langId} formatting provider`);
      }
    }
  }
